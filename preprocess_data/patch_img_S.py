...
#384为块进行切割，重叠部分长度为96
def SPARCS_patch(opt):
    raw_data_path = opt.data_dir_S
    crop_data_path = opt.data_crop_dir_S
    if opt.train == 'train':
        ...
        with open('./data/SPARCS/train.txt', 'r+')as f:
        ...
        with open('./data/SPARCS/val.txt', 'r+')as f:
            mss_name_list = f.readlines()
        f.close()
        ...
        with open('./data/SPARCS/test.txt', 'r+')as f:
            mss_name_list = f.readlines()
        f.close()
    mss_name_list = [x.split(' ')[0] for x in mss_name_list]
    dem_name_list = [x[:-9]+'_dem.tif' for x in mss_name_list]
    label_name_list = [x[:-9]+'_mask.png' for x in mss_name_list]
    spilt_window = opt.split_window
    patch_size = opt.patch_size
    overlap_size = opt.overlapping_size
    for i in tqdm(range(len(dem_name_list))):
        mss_data = gdal.Open(os.path.join(raw_data_path,'SPARCS_raw',mss_name_list[i]))
        label_data = gdal.Open(os.path.join(raw_data_path,'SPARCS_raw', label_name_list[i]))
        dem_data = gdal.Open(os.path.join(raw_data_path,'dem', dem_name_list[i]))
        x_size = mss_data.RasterXSize
        y_size = mss_data.RasterYSize
        #384 n - (n-1) 96 = 1000+pad
        ...
        label_path = os.path.join(crop_label_path,label_name_list[i][:-4]+'.tif')
