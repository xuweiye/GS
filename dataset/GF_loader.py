...
#     #时间查询
#     dataset = LevirCS_MSS_CloudSnowDataset(r'D:\同步文件\BaiduSyncdisk\project_plus\my_net\data\LevirCS\raw\train', None,None)
...
#
#         data, label = dataset.__getitem__(i)
#         if (data[data!=data].size(0) > 0 ):
#             print('nan')
#         if (label[label!=label].size(0)>0):
#             print('nan')
    #     sum0 = torch.sum(label == 0)
    #     sum1 = torch.sum(label == 1)
    #     sum2 = torch.sum(label== 2)
    #     if sum0 + sum1 + sum2 != 224 * 224:
    #         c = c+1
    #     if k !=c:
    #         print(c)
    # print(c)
    # root = '../data/LevirCS/raw/train'
    # mss_root = os.path.join(root,'mss')
    # mask_root = os.path.join(root,'mask')
    # dem_root = os.path.join(root, 'dem')

    #mss
    # mss_name_list = os.listdir(mss_root)
    #
    # for i in range(len(mss_name_list)):
    #     mss = gdal.Open(os.path.join(mss_root,mss_name_list[i]))
    #     for j in range(0,4):
    #         data = mss.GetRasterBand(j + 1).ReadAsArray()
    #         if data.shape!=(1340,1200):
    #             print(data.shape)
    #         print(data.shape[0])
    #     for j in range(1,5):
    #         mss_data = mss.GetRasterBand(j).ReadAsArray()
            # sum_mean_0 += np.mean(mss_data)
            # print(np.max(mss_data))
        # sum_mean_0 = sum_mean_0/4
        # sum_mean_1 +=sum_mean_0
        # print(sum_mean_1)
    # sum_mean_1 = sum_mean_1/len(mss_name_list)
    # print(sum_mean_1)

    #dem
    # max_name = 0
    # dem_name_list = os.listdir(mask_root)
    # for i in range(len(dem_name_list)):
    #     dem = gdal.Open(os.path.join(mask_root,dem_name_list[i]))
    #     dem_data = dem.ReadAsArray()
    #     if dem_data.shape!=(1340,1200):
    #         print(dem_data.shape)
    #     if np.max(dem_data)>max_name:
    #         max_name = np.max(dem_data)
    #         print(max_name)